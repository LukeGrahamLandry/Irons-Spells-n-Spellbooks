...
public abstract class AbstractSpellCastingMob extends CreatureEntity implements IAnimatable, IMagicEntity {
    ...
    public void setSyncedSpellData(SyncedSpellData syncedSpellData) {
        ...
        if (!playerMagicData.isCasting() && isCasting) {
            ...
            if (castingSpell.getSpell().getCastType() == CastType.INSTANT) {
                ...
                castingSpell.getSpell().onClientPreCast(level, castingSpell.getLevel(), this, Hand.MAIN_HAND, playerMagicData);
                castComplete();
            }
        }
    }
    ...
    public boolean setTeleportLocationBehindTarget(int distance) {
        ...
        if (target != null) {
            ...
            for (int i = 0; i < 24; i++) {
                ...
                teleportPos = Utils.moveToRelativeGroundLevel(level, target.position().subtract(new Vector3d(0, 0, distance / (float) (i / 7 + 1)).yRot(-(target.getYRot() + i * 45) * MathHelper.DEG_TO_RAD)).add(randomness), 5);
